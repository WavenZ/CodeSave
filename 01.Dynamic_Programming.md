...
最优子结构：假设硬币有`{1， 2， 5}`三种，需要换的钱为`amount`。目标是将`amount`换成以三种硬币，要求总的硬币个数最少。设将`amount`换成硬币的最少个数为`f(amount)`，则`f(amount) = 1 + min(f(amount - 1), f(amount - 2), f(amount - 5)`。因此要求解原问题`f(amount)`必须求的三个子最优解：`f(amount - 1)`、`f(amount - 2)`、`f(amount - 5)`，满足最优子结构性质。

重叠子问题：子问题为：`f(amount - 1)`、`f(amount - 2)`、`f(amount - 5)`，显然`f(amount - 1)`有子问题`f(amount - 2)`、`f(amount - 3)`、`f(amount - 2)`，满足子问题重叠性质。
		...
		for (int i = 1; i <= amount; i++) {
			...
			for (int coin : coins) {
				if ((i - coin >= 0) && (dp[i - coin] != -1))
					least = min(least, 1 + dp[i - coin]);
			}
			least != INT32_MAX ? dp[i] = least : dp[i] = -1;
		}
		return dp[amount];
	}
};
